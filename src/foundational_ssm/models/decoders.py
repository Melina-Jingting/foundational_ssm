...
class SSMFoundationalDecoder(eqx.Module):
    ...
    def __init__(
        ...
        for gi in range(num_dataset_groups):
            encoder = eqx.nn.Linear(
                input_dim, ssm_io_dim, key=enc_keys[gi]
            )
            self.encoders.append(encoder)

        # Decoder
        self.decoder = eqx.nn.Linear(
            ssm_io_dim, output_dim, key=decoder_key
        )
    ...
    def __call__(self, x, state, group_idx, key):
        ...
        if self._context_dim > 0:
            ...
            broadcast_context = jnp.broadcast_to(
                context_vec, (x.shape[0],) + context_vec.shape
            )
...
class SSMDownstreamDecoder(eqx.Module):
    ...
    def __call__(self, x, state, key):
        ...
        context_vec = jnp.broadcast_to(
            self.context_embedding, (x.shape[0],) + self.context_embedding.shape
        )
