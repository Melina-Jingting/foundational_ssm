...
from foundational_ssm.constants import DATA_ROOT, DATASET_GROUP_INFO
from foundational_ssm.dataset import TorchBrainDataset
from foundational_ssm.transform import (
    transform_brainsets_regular_time_series_smoothed,
    parse_session_id,
)
...
def get_brainset_data_loader(
    ...
    max_neural_units = int(
        np.max(
            [
                DATASET_GROUP_INFO[parse_session_id(k)]["max_num_units"]
                for k in sampling_intervals.keys()
            ]
        )
    )
    dataset.transform = partial(
        ...
        max_neural_units=max_neural_units,
    )
    total_window_length = (
        sampler_args.get("window_length", sampler_args.get("max_window_length", 1))
        + prepend_history
    ...
    loader = DataLoader(
        ...
        collate_fn=partial(
            pad_collate, fixed_seq_len=int(total_window_length * sampling_rate)
        ),  # the collator
        pin_memory=True,
        **dataloader_args,
    )
...
def get_brainset_train_val_loaders(
    ...
    train_dataset, train_loader, train_max_neural_units = get_brainset_data_loader(
        **train_loader_cfg,
    ...
    val_dataset, val_loader, val_max_neural_units = get_brainset_data_loader(
        **val_loader_cfg,
        dataset_args=dataset_args,
        split=f"{phase}_trial",
        prepend_history=prepend_history,
        data_root=data_root,
    )
