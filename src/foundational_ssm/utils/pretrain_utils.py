...
def validate_one_epoch(val_loader, model, state, skip_timesteps=0):
    ...
    all_session_dates = jnp.concatenate(
        all_session_dates, axis=0
    )  # 3. Concatenate session dates
    ...
    for dataset_group_idx in unique_dataset_group_idxs:
        ...
        for session_date in unique_session_dates:
            ...
            metrics[f"val/r2/{dataset_group_short_name}/{session_date}"] = float(
                r2_score
            )
        ...
        if session_r2_scores:  # Avoid division by zero if a group has no sessions
            metrics[f"val/r2/{dataset_group_short_name}/mean"] = np.mean(
                session_r2_scores
            )
            metrics[f"val/r2/{dataset_group_short_name}/std"] = np.std(
                session_r2_scores
            )
    ...
    metrics["val/r2/avg"] = float(
        np.mean([v for k, v in metrics.items() if "/mean" in k])
    )
...
def load_training_state(
    ...
    if wandb_resume_run_id is not None:
        ...
        wandb_run_name = (
            f"{model_name}_{dataset_name}{getattr(cfg.wandb, 'run_name_postfix', '')}"
        )
        config_dict = OmegaConf.to_container(cfg, resolve=True)
        wandb.init(
            project=cfg.wandb.project, name=wandb_run_name, config=dict(config_dict)
        )
    ...
    if wandb_resume_run_id is not None:
        artifact_full_name = (
            f"{cfg.wandb.entity}/{cfg.wandb.project}/{wandb_run_name}_checkpoint:latest"
        )
        ...
        current_step = checkpoint_metadata.get(
            "step", checkpoint_metadata.get("current_step", 0)
        )
