...
def get_nlb_datasets(
    dataset_cfg,
    rng_key,
    directory="/cs/student/projects1/ml/2024/mlaimon/data/foundational_ssm/processed/nlb",
    return_full=False,
):
    ...
    if dataset_cfg.phase == "test":
        ...
        train_data["neural_input"] = smooth_spikes(
            train_data["neural_input"], kern_sd_ms=20, bin_size_ms=5, time_axis=1
        )
        val_path = os.path.join(directory, dataset_cfg.name + "_val.h5")
        val_data = h5_to_dict(val_path)
        val_data["neural_input"] = smooth_spikes(
            val_data["neural_input"], kern_sd_ms=20, bin_size_ms=5, time_axis=1
        )

    if return_full:
        data = {
            "neural_input": np.concatenate(
                (train_data["neural_input"], val_data["neural_input"])
            ),
            "behavior_input": np.concatenate(
                (train_data["behavior_input"], val_data["behavior_input"])
            ),
...
def mse_loss_downstream(model, state, inputs, targets, mask, key, skip_timesteps):
    ...
    )(inputs, state, batch_keys)

    # Only evaluate loss on timesteps > skip_timesteps for causal model
    preds = preds[
        :, skip_timesteps:, :
    ]  # Shape: (batch, seq_len - skip_timesteps, output_dim)
...
def train_one_batch(
    ...
    (loss_value, state), grads = loss_fn(
        model, state, inputs, targets, mask, rng_key, skip_timesteps
    )
...
def train_one_epoch(
    ...
    train_loader = create_dataloader(
        train_data, batch_size, shuffle=True, rng_key=rng_key
    )
    epoch_loss = 0.0
    num_batches = 0

    for i, minibatch in enumerate(train_loader):
        rng_key, batch_train_key = jr.split(
            rng_key
        )  # Generate a new key for each batch
...
def train_one_epoch_brainsets(
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        batch_count, minute_start_time = track_batch_timing(
            batch_count, minute_start_time, current_time, current_step
        )
        prev_time = time.time()
        current_step += 1
...
def log_predictions_and_activations(
    model, state, data, cfg, epoch, current_step, run_name, dataset_name
):
...
def transfer_foundational_to_downstream(foundational_model, downstream_model):
    ...
    downstream_model = eqx.tree_at(
        lambda m: m.ssm_blocks, downstream_model, foundational_model.ssm_blocks
    )

    # Transfer decoder
    downstream_model = eqx.tree_at(
        lambda m: m.decoder, downstream_model, foundational_model.decoder
    )
...
def create_model_and_state(
    ...
    if hasattr(model_cfg, "checkpoint"):
        ...
        downstream_model, downstream_state = eqx.nn.make_with_state(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            foundation_model = eqx.tree_deserialise_leaves(
                os.path.join(temp_dir, "model.ckpt"), foundation_model
            )
        downstream_model = transfer_foundational_to_downstream(
            foundation_model, downstream_model
        )
...
def load_training_state(
    ...
    if hasattr(cfg.model, "checkpoint"):
        ...
        if cfg.training.from_scratch == False:
            with tempfile.TemporaryDirectory() as temp_dir:
                artifact.download(temp_dir)
                foundation_model, foundation_state = eqx.nn.make_with_state(
                    foundational_model_cls
                )(**foundational_run_cfg.model)
                foundation_model = eqx.tree_deserialise_leaves(
                    os.path.join(temp_dir, "model.ckpt"), foundation_model
                )
                foundation_state = eqx.tree_deserialise_leaves(
                    os.path.join(temp_dir, "state.ckpt"), foundation_state
                )
            downstream_model = transfer_foundational_to_downstream(
                foundation_model, downstream_model
            )
            downstream_state = transfer_state_by_shape(
                foundation_state, downstream_state
            )
