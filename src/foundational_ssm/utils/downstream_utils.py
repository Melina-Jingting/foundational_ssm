...
def validate_one_epoch(data, model, state, skip_timesteps):
    metrics = {} 
    val_start_time = time.time()

    data = {k: jax.device_put(np.array(v)) for k, v in data.items()}
    inputs = data["neural_input"]
    targets = data["behavior_input"]
    ...
    preds = jnp.where(mask, preds, 0)[:, skip_timesteps:, :]
    targets = jnp.where(mask, targets, 0)[:, skip_timesteps:, :]

    r2 = r2_score(preds.reshape(-1, target_dim), targets.reshape(-1, target_dim))
