...
def log_model_params_and_grads_wandb(model, grads=None):
    ...
    for path, value in model_params:
        if eqx.is_array(value):
            ...
            wandb.log({f"params/{full_path}": hist})
    for path, value in grads:
        if eqx.is_array(value):
            full_path = "".join(str(p) for p in path)
            hist = wandb.Histogram(value.flatten())
            wandb.log({f"grads/{full_path}": hist})
...
def load_checkpoint_wandb(
    model_template, state_template, opt_state_template, artifact_full_name
):
    ...
    try:
        ...
        raise FileNotFoundError(
            f"Could not find checkpoint artifact: {artifact_full_name}"
        )

    with tempfile.TemporaryDirectory() as temp_dir:
        artifact.download(temp_dir)
        model = eqx.tree_deserialise_leaves(
            os.path.join(temp_dir, "model.ckpt"), model_template
        )
        state = eqx.tree_deserialise_leaves(
            os.path.join(temp_dir, "state.ckpt"), state_template
        )
        try:
            opt_state = eqx.tree_deserialise_leaves(
                os.path.join(temp_dir, "opt_state.ckpt"), opt_state_template
            )
        except:
...
def load_h5_artifact_with_tempdir(
    artifact_name, artifact_type="predictions_and_activations"
):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not h5_files:
            ...
            raise FileNotFoundError(
                f"No H5 file found in {temp_dir}. Available files: {os.listdir(temp_dir)}"
            )
...
def default_deserialise_filter_spec(f: BinaryIO, x: Any) -> Any:
    ...
    **Returns**
    ...
    try:
        ...
        print(
            "Failed to load data for leaf with shape/ value:",
...
def load_model_and_state_from_checkpoint_wandb(
    ...
    model_template, state_template = eqx.nn.make_with_state(model_cls)(**model_cfg)

    with tempfile.TemporaryDirectory() as temp_dir:
        artifact.download(temp_dir)
        model = eqx.tree_deserialise_leaves(
            os.path.join(temp_dir, "model.ckpt"),
            model_template,
            default_deserialise_filter_spec,
        )
        state = eqx.tree_deserialise_leaves(
            os.path.join(temp_dir, "state.ckpt"),
            state_template,
            default_deserialise_filter_spec,
        )
