...
def pad_collate(batch, fixed_seq_len=None):
    ...
    neural_inputs = [
        item["neural_input"].squeeze(0) for item in batch if item is not None
    ]  # (timesteps, units)
    behavioral_inputs = [
        item["behavior_input"].squeeze(0) for item in batch if item is not None
    ]
    ...
    padded_neural = torch.stack(
        [pad_or_truncate(x, max_len) for x in neural_inputs if x is not None]
    )  # (batch, max_len, units)
    padded_behavior = torch.stack(
        [pad_or_truncate(x, max_len) for x in behavioral_inputs if x is not None]
    )
