...
from foundational_ssm.samplers import GroupedRandomFixedWindowSampler
from foundational_ssm.loaders import (
    get_dataset_config,
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("dataloading_debug.log"), logging.StreamHandler()],
)
...
def main():
    ...
    logger.info(
        f"Sampling intervals created - {len(train_sampling_intervals)} intervals"
    )
    ...
    for i, batch in tqdm(enumerate(train_loader), total=num_batches_to_test):
        ...
        if (i + 1) % 10 == 0:
            logger.info(
                f"Loaded {i + 1}/{num_batches_to_test} batches, last batch took {batch_time:.4f}s"
            )
    ...
    print("=" * 50)
    print(
        f"Transform: transform_brainsets_to_fixed_dim_samples_with_binning_and_smoothing"
    )
    ...
    outliers = loading_times[
        (loading_times < lower_bound) | (loading_times > upper_bound)
    ]
    print(
