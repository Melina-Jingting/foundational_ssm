...
processed_data_folder = (
    "/cs/student/projects1/ml/2024/mlaimon/data/foundational_ssm/processed/nlb"
)
...
conf = {
    ...
    "training": {"batch_size": 64, "epochs": 2000},
    "device": "cuda",
}
...
spikes = np.concat(
    [dataset_dict["train_spikes_heldin"], dataset_dict["train_spikes_heldout"]], axis=2
)
smoothed_spikes = torch.tensor(
    smooth_spikes(spikes, kern_sd_ms=40, bin_width=5), dtype=torch.float64
).to(device)
behavior = torch.tensor(dataset_dict["train_behavior"], dtype=torch.float64).to(device)
...
wandb.init(project="foundational_ssm_nlb", name=run_name, config=conf)
