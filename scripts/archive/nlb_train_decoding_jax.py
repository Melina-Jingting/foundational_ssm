...
def predict_batch(model, state, inputs, key):
    ...
    preds, _ = jax.vmap(model, axis_name="batch", in_axes=(0, None, 0))(
...
def make_step(model, filter_spec, inputs, targets, state, opt, opt_state, key):
    ...
    (value, state), grads = mse_loss(
        model_params, model_static, state, inputs, targets, key
    )
...
def main(cfg: DictConfig):
    ...
    processed_data_path = os.path.join(
        to_absolute_path(cfg.dataset.processed_data_folder), cfg.dataset.name + ".h5"
    )
    trial_info_path = os.path.join(
        to_absolute_path(cfg.dataset.processed_data_folder), cfg.dataset.name + ".csv"
    )
    ...
    smoothed_spikes = torch.tensor(
        smooth_spikes(spikes, kern_sd_ms=40, bin_width=5), dtype=torch.float64
    )
    behavior = torch.tensor(dataset_dict["train_behavior"], dtype=torch.float64)
    ...
    if cfg.model.freeze_ssm:
        ...
        filter_spec = eqx.tree_at(
            where, filter_spec, replace=(False,) * len(filter_spec.blocks)
        )
