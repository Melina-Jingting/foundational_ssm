...
def main():
    ...
    for recording_id in tqdm(recording_ids):
        ...
        with h5py.File(input_path) as f:
            ...
            regular_hand, regular_hand_times = (
                recording_data.cursor.get_regular_time_series_array(
                    sampling_rate, "vel"
                )
            )
            vel_regular = RegularTimeSeries(
                ...
                domain=Interval(
                    start=min(regular_hand_times), end=max(regular_hand_times)
                ),
            )

            binned_spikes, binned_spike_times = (
                recording_data.spikes.get_regular_time_series_array(
                    sampling_rate, "unit_index", is_index=True
                )
            )
            smoothed_spikes = smooth_spikes(
                binned_spikes, kern_sd_ms=20, bin_size_ms=5, time_axis=0
            )
            ...
            smoothed_binned_spikes = RegularTimeSeries(
                ...
                sampling_rate=sampling_rate,
                domain=Interval(
                    start=min(binned_spike_times), end=max(binned_spike_times)
                ),
            )
