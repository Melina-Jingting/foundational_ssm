...
def visualize_discrete_eigenvalues(
    ...
    for i, (model_name, lambdas) in enumerate(lambdas_disc_dict.items()):
        ...
        # Plot eigenvalues
        ax.scatter(
            lambdas.real, lambdas.imag, 
            s=50, color=colors[model_name], alpha=0.8, 
        ...
        unit_circle = Circle((0, 0), 1, fill=False, color='#D62828', linestyle='--', linewidth=2, alpha=0.7)
        ...
        ax_individual.scatter(
            lambdas.real, lambdas.imag, 
            s=60, color=colors[model_name], alpha=0.8, 
            edgecolors='black', linewidths=0.5
        )
        
        # Add grid and axes to individual plot
        ax_individual.axhline(y=0, color='black', linestyle='--', alpha=0.3, linewidth=1)
        ax_individual.axvline(x=0, color='black', linestyle='--', alpha=0.3, linewidth=1)
        ...
        unit_circle_individual = Circle((0, 0), 1, fill=False, color='#D62828', 
        ...
        if save_path:
            individual_path = save_path.replace('.png', f'_{model_name.lower().replace("-", "_")}.png')
            plt.savefig(individual_path, dpi=300, bbox_inches='tight', transparent=False)
    ...
    if title:
        ...
        fig.suptitle(f'Discrete-time Eigenvalues (dt={config["timestep"]})', fontsize=20)
    ...
    for model_name, lambdas in lambdas_disc_dict.items():
        ax_combined.scatter(
            lambdas.real, lambdas.imag, 
            s=50, color=colors[model_name], alpha=0.8, 
            label=model_name, edgecolors='black', linewidths=0.5
        )
    ...
    ax_combined.set_title(f'Combined Discrete-time Eigenvalues (dt={config["timestep"]})', fontsize=18)
...
def extract_s5_eigenvalues():
    ...
    # Extract eigenvalues
    Lambda_re = np.array(s5_model.__getstate__()['blocks'][0].__getstate__()['ssm'].Lambda_re)
    Lambda_im = np.array(s5_model.__getstate__()['blocks'][0].__getstate__()['ssm'].Lambda_im)
...
def extract_linoss_im_eigenvalues():
    ...
    for i, (a, dt) in enumerate(zip(A_diag, steps)):
        ...
        schur_comp = 1. / (1. + dt**2 * a)
        m11 = 1. - dt**2 * a * schur_comp
        m12 = -1. * dt * a * schur_comp
        m21 = dt * schur_comp
        m22 = schur_comp
