...
def main(cfg: OmegaConf):
    ...
    for epoch in range(0, cfg.training.epochs):
        ...
        if epoch % cfg.training.log_val_every == 0:
            ...
            metrics = validate_one_epoch_brainsets(val_loader, model, state, cfg.dataset.skip_timesteps)
            metrics['epoch'] = epoch
        ...
        model, state, opt_state, current_step, epoch_loss = train_one_epoch_brainsets(
            train_loader, model, state, mse_loss_downstream, opt, opt_state, subkey, lr_scheduler, current_step, epoch, cfg.dataset.skip_timesteps
        )
